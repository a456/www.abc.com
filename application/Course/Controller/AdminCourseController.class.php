...
class AdminCourseController extends AdminbaseController {
	...
	function add_post(){
		if (IS_POST) {
			...
			$data['cs_brief']=htmlspecialchars_decode($_POST['cs_brief']);
	...
	public function edit_post(){
		if (IS_POST) {
			...
			$data = $this->course_obj->create(); 
			$data['cs_brief']=htmlspecialchars_decode($_POST['cs_brief']);
			$result=$this->course_obj->where("id=$id")->save();
			if ($result) {
	...
	private  function _lists($status=1){
		...
		$where_ands=empty($term_id)?array("cs_state<=$status"):array("ty_id = $term_id and cs_state=$status");
		$fields=array(
